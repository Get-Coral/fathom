	...
	return (
		<main className="min-h-screen bg-abyss px-6 py-10 text-ink sm:px-8 lg:px-12">
			<div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.9fr_1.1fr]">
				<section className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-moss/12 via-white/[0.04] to-coral/10 p-8">
					...
					<div className="mt-10 space-y-4">
						{[
							...
							"Username and password are optional, but help validate richer access when provided.",
							"Saved settings live in Fathom's local SQLite database.",
							summary.source === "env"
								? "Current values are coming from environment variables until you save an override here."
								: summary.source === "database"
									? "Current values are already being served from Fathom's local SQLite database."
				...
				<section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-8">
					<form className="space-y-5" onSubmit={handleSubmit}>
						<div>
							...
							<input
								id="setup-url"
								className="w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-base text-ink outline-none transition focus:border-moss/40"
								value={url}
						...
						<div className="flex flex-wrap items-center gap-3 pt-3">
							<button
								type="submit"
								disabled={saving}
								className="rounded-full bg-moss px-6 py-3 text-sm font-semibold text-abyss transition hover:bg-[#a7d7bd] disabled:cursor-not-allowed disabled:opacity-60"
							>
								{saving ? "Connecting…" : "Connect Jellyfin"}
							</button>
							<span className="text-sm text-ink-faint">
								Fathom validates the connection before saving it.
							</span>
						</div>
					</form>
				</section>
